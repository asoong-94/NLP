...
class CustomModel:
  ...
  def train(self, corpus):
    ...
    for sentence in corpus.corpus:
      ...
      for i in range(0, len(sentence.data)-1):
        # bigram is this word and prev hashed together
      ...
      for i in range(0, len(sentence.data)-2):
        # trigram is this word, prev and prev prev hashed together
        first = sentence.data[i].word
        second = sentence.data[i+1].word
  ...
  def score(self, sentence):
    ...
    for i in range(0, len(sentence)-2):
      ...
      # if trigram has count then take trigram count
      if trigramCount > 0:
        score += math.log(trigramCount)
        score -= math.log(bigramCount)
      # elif no trigram then take bigram 
      elif bigramCount > 0:
        score += math.log(bigramCount)
        score -= math.log(prevCount)
      # if no trigram and no bigram count, then take unigram 
      else:
